...
. Decision trees learn rules of the form **"if AND b AND c, THEN y."**
. Tree can express non-linear concepts like intervals and non-monotonic relationship, because
  AND is similar to multiplication, trees can express certain kinds of non-additive interactions.
  
. PRO: Decision trees have an expressive concept space 
        -> a tree fits the intelligence data better than a linear model if their predictions have lower RMSE)
        
. CON: 
      - Trees also give only coarse-grained prediction while linear can give continuous valued predictions
      - Trees express axis-aligned regions -> hard to express truly linear relationship
      
      --> Can make finer grained predictions by doing more splits on the data to build deeper trees, 
          but a deeper tree can be overly complex, potentially overfitting the training data.
      
      --> Shallower tree with too few splits are less likely to overfit, but the predictions are often too
          coarse-grained

. *An ensemble* - a model made up several trees 
              - will give finer-grained predictions and usually better quality models than a single tree.
...
. Random forests try to resolve the issues with decision tree model by building multiple trees from the training data.
  - Using slightly different data to build each tree adds diversity to the models.
...
. For a single tree, each node is formed by picking a variable to split the data on, and a value to make the split.


__Building a Random Forest Model__
. In a random forest, the set of candidate variables to consider is randomly selected 
      -> diversity to the set of trees
. After a trees are grown, the model make a prediction on a datum by running through all the trees and averaging the result
  
                1. Draw bootstrapped sample from training data
                2. For each sample grow a tree
                  . At each node, pick the best variable to split on (from a random subset of all variables)
                  . Continue until tree is grown
                3. To score a datum, evaluate it with all the trees and average the results.
                
__Random Forests with ranger()__
              
              model <- ranger(formyla, trainingdata, num.trees = 500, respect.unordered.factors = "order")
              
                The ranger() function takes a formula, the training dat and the number of trees
                If the outcome variable is a numeric value, ranger will automatically do regression rather
                than classification.
                By default, ranger builds 500 trees
                  . formula, trainingdata
                  . num.trees                    - (default 500) - recommend use at least 200
                
                  . mtry                         - number of variables to try at each node
                                                 - default: square root of the total number of variables
                                                 
                  . respect.unordered.factors    - tells ranger how to treat categorical variables
                                                 - recommend set to "order"
                                                      - causes ranger to safely and meaningfully encode the
                                                        categorical variables as numbers. This encoding run faster
                                                        than converting a categorical variable to indicator variable
                                                        when the categorical variable has a very large number of
                                                        possible values
                                                 - "safe" hashing of categorical variables
        

Printing a ranger model will display what's called an out of bag estimate (OOB) of the model's R-squared
and mean squared error (MSE) --> the estimates of how the model will perform on future data.
When possible, you should still evaluate the model directly on test data.

__Predicting with a ranger() model__
                
                predict(model, testdata)$predictions
                
                  The predict function for a ranger model takes the model and a new data set.
                  It returns an object with a field called predictions, containing the predicted outcomes

__Evaluating the model__

                Calculate RMSE
                
                  testdata %>% mutate(residual = pred - cnt) %>%
                               summarize(rmse = sqrt(mean(residual^2)))

__Graph__ 
    Scatterplot
    Or plotting the data as a function of time
    
**Practice**
Build a random forest model for bike rentals

In this exercise you will again build a model to predict the number of bikes rented in an hour as a function 
of the weather, the type of day (holiday, working day, or weekend), and the time of day. You will train the 
model on data from the month of July.

You will use the ranger package to fit the random forest model. For this exercise, the key arguments to 
the ranger() call are:
...
In this exercise you will use the model that you fit in the previous exercise to predict bike rentals for the 
month of August.

The predict() function for a ranger model produces a list. One of the elements of this list is predictions, 
a vector of predicted values. You can access predictions with the $ notation for accessing named elements of 
a list:
...
In the previous exercise, you saw that the random forest bike model did better on the August data than 
the quasiposson model, in terms of RMSE.

In this exercise you will visualize the random forest model's August predictions as a function of time. 
The corresponding plot from the quasipoisson model that you built in a previous exercise is in the workspace 
for you to compare.
...
__Why convert categoricals manually?__
. Most R functions manage the conversion for you
    - model.matrix()
. xgboost() - a package that comes from Python - does not
    --> must convert categorical variables to numeric representation
. Conversion to indicators: one-hot encoding

__One-hot-encoding and data cleaning with 'vtreat'__
. 'vtreat' also cleans up missing values in both categorical and numeric data.
. Basic idea:
      - designTreatmentsZ() to design a treatment plan from the training data, 
                              --> records the steps needed to safely one-hot-encode with training and future data
                              then
      - prepare() converts the training and the future data to a form that is compatible with xgboost
...
. Example
                  ...
                  Having training data and test data
                      - x : categorical input variable
                      - u : numerical input variable
                      - y : outcome
                  ...
                  2. Get the new variables
                      - The treatment plan contains a member called the score frame, which hold a mapping from
                        the original variable names to new variable names, and the type of the new variables
                      
                    (scoreFrame <- treatplan$scoreFrame %>%
                    +         select(varName, origName, code))
                    
                      - "lev" variables - indicator variable
                      - "clean" variables - numeric variables, cleaned to not hold bad values like NA or NaN
                      - We can get a list of the new variable names from the scoreFrame
                  ...
                  3. Prepare the training data for modeling
                      - Once we have the variable names, we can use the prepare function and the treatment plan
                        to create new treated training data suitable for training an xgboost model.
                  ...
                  4. Treated data is all numerical, with no missing values.
                      The outcome variable is not present in the treated data.
                      
                  5. Preapre the test data before model application
                      We must also use prepare on future data before model application.
                      
                      (test.treat <- prepare(treatplan, test, varRestriciton =  newvars))
                      
__vtreat treatment is robust__
Sometimes, there are levels of categorical varibale that don't appear in the training data --> crash the model
--> vtreat encodings handle this situation gracefully

vtreat can also do a meaningful encoding of categorical variables similar to the method ranger() uses

-----------------------------------------------------------------------------------------------------------

vtreat on a small example
In this exercise you will use vtreat to one-hot-encode a categorical variable on a small example. vtreat creates
a treatment plan to transform categorical variables into indicator variables (coded "lev"), and to clean bad
values out of numerical variables (coded "clean").
...
designTreatmentsZ() returns a list with an element scoreFrame: a data frame that includes the names and types 
...
- Print dframe. We will assume that color and size are input variables, and popularity is the outcome 
    to be predicted.
...
- Use prepare() to create a new data frame dframe.treat that is a one-hot-encoded version of dframe 
...
>[1] "vtreat 1.2.0 inspecting inputs Thu May 28 07:28:38 2020"
[1] "designing treatments Thu May 28 07:28:38 2020"
[1] " have initial level statistics Thu May 28 07:28:38 2020"
[1] "design var color Thu May 28 07:28:38 2020"
[1] "design var size Thu May 28 07:28:38 2020"
[1] " scoring treatments Thu May 28 07:28:38 2020"
[1] "have treatment plan Thu May 28 07:28:38 2020"
...
The treatment plan treatplan and the set of variables newvars from the previous exercise are still in 
your workspace. dframe and a new data frame testframe are also in your workspace.

- Print dframe and testframe.
    . Are there colors in testframe that didn't appear in dframe?
- Call prepare() to create a one-hot-encoded version of testframe (without the outcome). 
  Call it testframe.treat and print it.
...
- Load the package vtreat.
- Use designTreatmentsZ() to create a treatment plan treatplan for the variables in vars from bikesJuly 
  (the training data).
  . Set the flag verbose=FALSE to prevent the function from printing too many messages.
- Fill in the blanks to create a vector newvars that contains only the names of the clean and lev 
  transformed variables. Print it.
...
__How Gradient Boosting Works__ 

Gradient Boosting is an ensemble method that builds up a model by incrementally improving the existing one.

. Start the model by fitting a single, usually shallow, tree to the data
. Next, fit a tree to the residuals of the model and find the weighted sum of that tree with the first one 
  that gives the best fit
. For regularized boosting, decrease the learning by a factor eta - η, between 0 and 1. Eta close to 1 will give
  faster learning, but increases the risk of overfit. Smaller ete slows the learning, but lessens the risk of
  overfit
. Repeat until either the residuals are small enough, or the maximum number of iterations is reached.
          ...
          2. Fit a tree T2 to the residuals. Find gamma γ such that
                M2 = M1 + γT2 is the best fit to data
          ** Regularization: learning rate: η ∈ (0, 1) **
              M2 = M1 + ηγT2
...
__Cross-validation to guard against overfit__

Because graditent boosting optimizes error on the training data, it's very easy to overfit the model
Training error keeps decreasing but test error doesn't.
--> So best practice is to estimate out-of-sample error via cross-validation for each incremental model, 
    then retroactively decide how many trees to use
    
__Best practice (with xgboost())__

In xgboost, the xgb.cv() function fits a model and calculates the cross-validated errors.
Run this function with a large number of rounds

xgb.cv() records the estimated errors in the evaluation log element of its output.
use the evaluation_log to find the number of trees with the lowest estimated RMSE

Run the xgboost() function with the right number of trees to get the final model
...
Prepare the data
. Since xgboost can't work directly with categorical variables, we use vtreat to prepare the data

    treatplan <- designTreatmentsZ(bikesJan, vars)
    newvars <- treatplan$scoreFrame %>% filter(code %in% c("clean", "lev")) %>% use_serires(varName)
    bikeJan.treat <- prepare(treatplan, bikesJan, varRestriction = newvars)
...
Training a model with sgboost()/xgb.cv()
- First run xgb.cv, which takes a large number of arguments.

    cv <- xgb.cv(data = as.matrix(bikesJan.treat), label = bikesJan$cnt, objective = "reg:linear",
...
Key inputs to xgb.cv() and xgboost()
  ...
  . nfold (xgb.cv() only) : number of folds for cross - to estimate out-of-sample error
  
Find the right number of trees
- The out-of-sample error estimates are in the column test_rmse_mean of the evaluation log.
- The index of the minimum value corresponds to the best number of trees

    elog <- as.data.frame(cv$evaluation_log)
    (nrounds <- which.min(elog$test_rmse_mean))
     -----> outcome = 78
    
Run xgboost() with the right number of trees to get the final model

    nrounds <- 78
    
    model <- xgboost(data = as.maxtrix(bikesJan.treat), label = bikesJan$cnt, nrounds = nrounds,
                                                        objective = "reg:linear", 
                                                        eta = 0.3,
                                                        depth = 6)

Predict with an xgboost() model
- The predict function for xgboost models takes a model and the input data
    
    # prepare the test data to use vtreat
...
In this exercise you will get ready to build a gradient boosting model to predict the number of bikes rented 
in an hour as a function of the weather and the type and time of day. You will train the model on data from 
the month of July.
...
You will use the xgboost package to fit the random forest model. The function xgb.cv() uses cross-validation 
to estimate the out-of-sample learning error as each new tree is added to the model. The appropriate number 
of trees to use in the final model is the number that minimizes the holdout RMSE.
...
- Get the data frame evaluation_log from cv and assign it to the variable elog. Each row of the evaluation_log
  corresponds to an additional tree, so the row number tells you the number of trees in the model.
- Fill in the blanks to get the number of trees with the minimum value of the columns train_rmse_mean and
  test_rmse_mean.
...
> hr_lev_x.0 hr_lev_x.1 hr_lev_x.10 hr_lev_x.11 hr_lev_x.12 hr_lev_x.13
1          1          0           0           0           0           0
2          0          1           0           0           0           0
3          0          0           0           0           0           0
4          0          0           0           0           0           0
5          0          0           0           0           0           0
6          0          0           0           0           0           0
  hr_lev_x.14 hr_lev_x.15 hr_lev_x.16 hr_lev_x.17 hr_lev_x.18 hr_lev_x.19
1           0           0           0           0           0           0
2           0           0           0           0           0           0
3           0           0           0           0           0           0
4           0           0           0           0           0           0
5           0           0           0           0           0           0
6           0           0           0           0           0           0
  hr_lev_x.2 hr_lev_x.20 hr_lev_x.21 hr_lev_x.22 hr_lev_x.23 hr_lev_x.3
1          0           0           0           0           0          0
2          0           0           0           0           0          0
3          1           0           0           0           0          0
4          0           0           0           0           0          1
5          0           0           0           0           0          0
6          0           0           0           0           0          0
  hr_lev_x.4 hr_lev_x.5 hr_lev_x.6 hr_lev_x.7 hr_lev_x.8 hr_lev_x.9
1          0          0          0          0          0          0
2          0          0          0          0          0          0
3          0          0          0          0          0          0
4          0          0          0          0          0          0
5          1          0          0          0          0          0
6          0          1          0          0          0          0
  holiday_clean workingday_clean weathersit_lev_x.Clear.to.partly.cloudy
1             0                0                                       1
2             0                0                                       1
3             0                0                                       1
4             0                0                                       1
5             0                0                                       1
6             0                0                                       1
  weathersit_lev_x.Light.Precipitation weathersit_lev_x.Misty temp_clean
1                                    0                      0       0.76
2                                    0                      0       0.74
3                                    0                      0       0.72
4                                    0                      0       0.72
5                                    0                      0       0.70
6                                    0                      0       0.68
  atemp_clean hum_clean windspeed_clean
1      0.7273      0.66          0.0000
2      0.6970      0.70          0.1343
3      0.6970      0.74          0.0896
4      0.7121      0.84          0.1343
5      0.6667      0.79          0.1940
6      0.6364      0.79          0.1045

# The July data is in the workspace
ls()
> [1] "bikesJuly"       "bikesJuly.treat" "cv"              "elog"
...
In this exercise you will fit a gradient boosting model using xgboost() to predict the number of bikes rented 
in an hour as a function of the weather and the type and time of day. You will train the model on data from 
the month of July and predict on data for the month of August.

The datasets for July and August are loaded into your workspace. Remember the vtreat-ed data no longer has 
the outcome column, so you must get it from the original data (the cnt column).
...
ls()
>[1] "bike_model_xgb"    "bikesAugust"       "bikesAugust.treat"
[4] "bikesJuly"         "bikesJuly.treat"   "ntrees"
...
In this exercise you will evaluate the gradient boosting model bike_model_xgb that you fit in the last exercise,
using data from the month of August. You'll compare this model's RMSE for August to the RMSE of previous models 
that you've built.

The dataset bikesAugust is in the workspace. You have already made predictions using the xgboost model; they are 
in the column pred.


- Fill in the blanks to calculate the RMSE of the predictions.
    . How does it compare to the RMSE from the poisson model (approx. 112.6) and the random forest model 
      (approx. 96.7)?
